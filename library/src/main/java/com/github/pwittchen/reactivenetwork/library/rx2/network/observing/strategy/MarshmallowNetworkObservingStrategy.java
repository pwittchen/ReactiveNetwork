...
public class MarshmallowNetworkObservingStrategy implements NetworkObservingStrategy {
  ...
  // PLEASE NOTE: State of available networks is required to handle cases when user is connected to multiple networks,
  // but looses connection with one of them. In such case, only onLost event is generated by the NetworkCallback,
  // so we need to manually propagate event with the available connected network down the stream in onLost event.
  // With this approach user will always receive an information about connectivity, if there is any.
  // This behavior is consistent with the remaining NetworkObservingStrategies
